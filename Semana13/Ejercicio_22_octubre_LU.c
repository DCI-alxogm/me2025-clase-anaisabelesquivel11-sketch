...
int main() {
    ...
    for(i = 0; i < n; i++) { //inicializamos las matrices L y U :) 
        for(j = 0; j < n; j++) {
            if(i == j)
    ...
    for(i = 0; i < n; i++) { //factorización de LU
        
        for(j = i; j < n; j++) { //calculamos U
            U[i][j] = A[i][j];
            for(k = 0; k < i; k++) {
                U[i][j] -= L[i][k] * U[k][j];
            }
    ...
    for(i = 0; i < n; i++) { //con Ly=b hacemos la sustitución hacia adelante
        y[i] = b[i];
        for(j = 0; j < i; j++) {
            y[i] -= L[i][j] * y[j];
        }
    ...
    for(i = n-1; i >= 0; i--) { //hacemos la sustitución hacia atrás
        x[i] = y[i];
        for(j = i+1; j < n; j++) {
            x[i] -= U[i][j] * x[j];
        }
